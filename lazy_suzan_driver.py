...
def get_input(prompt, hints):  # This function gets a spoken user input
...
def list_items_on_table():
    ...
    for table_obj, ID in objects.items():  # For each obj & id in objects
        say(table_obj + ".")  # Print name of object in bullet point format
...
def goto():
    ...
    goto = get_input("Where to? ", objects.keys())  # Ask what item the person wants
    ...
    if not valid_answer_found:
        say("You can't go to that item or modifier now.")  # Give an error


def toggle():
    toggle = get_input("Item to Toggle? ", all_objects.keys())  # Ask for item
...
def edit():
    edit = get_input("Item to Edit? ", all_objects.keys())  # Ask for item
