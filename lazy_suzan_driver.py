...
def get_input(prompt = "Cancel", hints):  # This function gets a spoken user
    # input
    say(prompt)  # Speak the prompt
    return_value = CloudSpeechClient().recognize(language_code=en_AU,
                                                 hint_phrases=hints)  # Get
    # the input
...
def get_modifier():  # This function returns the modifier for the specified
    # user
    modifier = get_input("Modifier? ", range(0, 361))
    if modifier is None:
        return None
...
class TableObject:
    ...
    def goto(self, mod):
        ...
        if turn_is_negative:
            ...
            say("The table turned " + str(turn) + " anticlockwise.")
        elif turn == 0:  # i.e. table is already in correct position
            say("The table did not move.")
        else:
...
class LazySusan:  # Class to keep track of an emulated lazy susan
    ...
    def list_items(self):
        ...
        for table_obj, ID in objects.items():  # For each obj & id in
            # objects
            say(table_obj + ".")  # Print name of object in bullet point
            # format
    ...
    def turn(self):
        ...
        if valid_turn:
            ...
            say("The turn you entered wasn't an integer")
    ...
    def goto(self):
        ...
        if modifier is None:
            say("Cancelled")
            return
        goto = get_input("Where to? ", objects.keys())  # Ask what item the
        # person wants
        if goto is None:
            say("Cancelled")
            return
        ...
        if not valid_answer_found:
            say("You can't go to that item or modifier now.")  # Give an
            # error

    def toggle(self):
        toggle = get_input("Item to Toggle? ", self.all_objects.keys())  #
        # Ask for item
        if toggle is None:
            say("Cancelled")
            return
        ...
        if not toggle_item_exists:
            say("The item you asked for doesn't exist.")
        else:
    ...
    def edit(self):
        edit = get_input("Item to Edit? ", self.all_objects.keys())
        # Ask for item
        if edit is None:
            say("Cancelled")
            return
...
def main():
    ...
    while True:
        board.button.wait_for_press()  # Wait for the user to press the button
        # Ask what the user wants to do
        action = str(get_input("Action: ", ("Goto", "Toggle", "Edit", "Turn")))
